...
public class QuickSort {
    ...
    static void sort2(int[] array, int low, int high) {
        ...
        while (i < j) {
            ...
            //做完这个，则arr[i] <= pivot, arr[j] >= pivot，又回归到初始化的状态
            if (i < j) {
                swap(array, i, j);
            }
        }
        //此时满足初始化时的状态，且i >= j，可推得arr[j, i] == pivot，arr[low, j] <= x，arr[j+1, high] >= x
        //要小心边界问题，以免造成无穷分治，即sort(0,n)划分为sort(0,0)和sort(0,n)
        //j的取值范围是[low, high-1]，证明见下
        //假设 j 最终的值为 high ，说明只有一轮循环(两轮的话 j 至少会自减两次)
        //说明arr[high] <= x (因为要跳出do-while循环)
        //说明 i >= high(while循环的结束条件)， i 为 high 或 high + 1(必不可能成立)
        //说明 i 自增到了 high , 说明 arr[high] >= pivot 和 q[low, ..., high-1] < pivot,
        //得出 arr[high] = pivot 和 q[low, ..., high-1] < pivot 的结论,但这与 x = arr[low + high >> 1]矛盾。
        sort2(array, low, j);
